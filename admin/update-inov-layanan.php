    ...
    $keterangan = $_POST['keterangan'];
        ...
        $file_tmp = $_FILES['gambar']['tmp_name'];
            ...
            if(in_array($ekstensi, $eks_dibolehkan) === true) {
                ...
                $query = ("UPDATE inov_layanan SET keterangan='$keterangan', g_inovasi='$nama' WHERE id='$id'");
                $hasil = mysql_query($query);

                if($hasil) {
                    echo "<script>alert('Data berhasil Di Update'); window.location = 'inov-layanan.php'</script>";
                    } else {
                    echo "<script>alert('Data gagal Di Update');window.location = 'inov-layanan.php'</script>";
                    }
                    }else {
